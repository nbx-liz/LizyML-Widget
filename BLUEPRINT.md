...
│   inner_valid        [Default   ▼]  ← select     │
│                                                  │
│ ▸ Evaluation                                     │
│   metrics  [auc][binary_logloss][...]           │  ← chip (multi)
│                                                  │
...
| Inner Validation | `training.early_stopping.inner_valid` | `null` | UI は `Default` と backend schema が許可する variant（`holdout` / `group_holdout` / `time_holdout`）を表示できる。canonical value は `{method: ...}` を含む object または `null` とし、表示専用ラベル（例: `fold_0`）を保存してはいけない |
...
│   │ learning_rate        │ [Fixed|Range ] │ 1e-4 ~ 0.1  │
...
│   │ auto_num_leaves      │ [Fixed|Choice] │ ☑true □false │
│   │ num_leaves_ratio     │ [Fixed|Range ] │ 0.5 ~ 1.0    │ ← ON
│   │ num_leaves           │ [Fixed|Range ] │ 256          │ ← OFF / Choice時
│   │ min_data_in_leaf...  │ [Fixed|Range ] │ 0.01         │
│   │ min_data_in_bin...   │ [Fixed|Range ] │ 0.01         │
│   │ balanced             │ [Fixed|Choice] │ null         │
│   └──────────────────────┴────────────────┴──────────┘ │
│                                                  │
│   Log Output (verbose)  [-1       ]              │
│                                                  │
...
| Metric | `tuning.optuna.params.metric` | 初期 backend contract の task 別先頭 metric | Optuna が最適化する metric 名。`backend_contract.ui_schema.option_sets` に従うセグメントボタンで 1 クリック選択。direction は選択された metric に応じて backend が決定する。※ Search Space の `metric`（`tuning.optuna.space.metric`）とは別概念 |
| Search Space | `tuning.optuna.space` | `{}` | 空 dict 時は backend default space を使用 |
| Log Output | `model.params.verbose` | `-1` | Search Space の下に独立した大型 `- / +` ステッパー付き数値入力として表示。Fit と同じ扱い |

Search Space の行は `backend_contract.ui_schema.search_space_catalog` をもとに pre-populate する。初期モードはすべて **Fixed**（Fit の初期値をそのまま保持）。ユーザーが Range / Choice に変更したパラメータのみ `tuning.optuna.space` に追加される。Range/Choice が 0 件のときに empty `space={}` を許容するかどうかは `backend_contract.capabilities` で決める。初期 backend では empty `space={}` を UI / Python API の両方で許容する。

Tune Settings の `Metric` は **ドロップダウンではなくセグメントボタン**で表示し、候補は `backend_contract.ui_schema.option_sets` から解決する。
...
|-------|------|-----------|-------------------|--------------|
| `objective` | LGBMParam | Fixed | task 依存 | Fixed / Choice | Choice 時は `backend_contract.ui_schema.option_sets.objective[task]` のチップボタン（複数選択可）。`config.choices` に選択した `string[]` を格納 |
...
| `tuning.optuna.params.metric` | `string \| null` | No | UI Tune Settings → `patch_config` | Adapter `apply_config_patch()` | — |
