...
class BackendAdapter(Protocol):
    ...
    # P-030: コード出力
    def export_code(self, model: Any, path: str) -> "Path": ...
```
...
| `type` | ペイロード | 説明 |
|--------|-----------|------|
| `"plot_data"` | `{plot_type, plotly_json}` | プロット JSON |
...
class LizyWidget(anywidget.AnyWidget):
    ...
    def export_code(self, path: str | None = None) -> "Path":
        """学習済みモデルの推論コードを出力する。path 未指定時は tmpdir に出力。"""
```
...
| 要素 | 説明 | 初期値 |
|------|------|------|
| Strategy | `kfold` / `stratified_kfold` / `group_kfold` / `time_series` / `purged_time_series` / `group_time_series` / `blocked_group_kfold` のセグメントボタン | Task 依存（`binary` / `multiclass` → `stratified_kfold`、`regression` → `kfold`） |
...
| Blocks | `blocked_group_kfold` で表示。時間ブロック数 | `5` |
| Groups | `blocked_group_kfold` で表示。グループカラムのドロップダウン | `null` |
| Min train rows | `blocked_group_kfold` で表示。最小学習行数 | `null` |
| Min valid rows | `blocked_group_kfold` で表示。最小検証行数 | `null` |
...
| Blocks | `split.blocks` | `null` | `blocked_group_kfold` で使用 |
| Groups | `split.groups` | `null` | `blocked_group_kfold` で使用 |
| Min train rows | `split.min_train_rows` | `null` | `blocked_group_kfold` で任意 |
| Min valid rows | `split.min_valid_rows` | `null` | `blocked_group_kfold` で任意 |
...
| Blocks / Groups / Min train/valid rows | `split.blocks` / `split.groups` / `split.min_train_rows` / `split.min_valid_rows` |
...
│   ├── BlockedGroupKFold.tsx  # BlockedGroupKFold CV 設定 UI
│   ├── DistributionBar.tsx    # カラム分布バー
│   └── FoldPreview.tsx        # Split プレビュー表示
├── hooks/
